...
public class BlueMower : MonoBehaviour {
	...
	void FixedUpdate () {
		...
		if (GameManager.gameStarted) {
			...
			if (Input.GetKey("left") || Input.GetKey("right")) {
				...
				float rotAxis = (Input.GetKey("left")) ? -0.75f : 0.75f;
				yRotation += rotAxis * currentTurnRate;
				transform.eulerAngles = new Vector3(0,yRotation,0);
			}
			// based on input, determine requested action
			if (Input.GetKey("up")) { // requesting forward
				requestedDirection = direction.forward;
				isAccelerating = true;
			} else if (Input.GetKey("down")) { // requesting reverse
			...
			// FIXED Y POSITION
			GameObject blueMower = GameObject.Find("BlueMower");
			blueMower.transform.position = new Vector3(blueMower.transform.position.x, 0.5f, blueMower.transform.position.z);
		}
	}
	...
	void OnTriggerEnter(Collider other){
		...
		if (other.gameObject.name == "RedMower") {
			currentSpeed*=0.5f;
			yRotation+=120f;
		...
		if (other.gameObject.tag == "Rock") {
			if(currentSpeed>0.5f){
				GameManager.blueScore-=100;
				//GUIManager.SetBlueScore(GameManager.blueScore);
		...
		if (other.gameObject.tag == "PowerUp") {
			...
			case 1:
				...
				clipRadius = 2f;
				GUIManager.powerUpDisplay("BLUE GROWING!!!");
				break;
			case 2:
				scale = otherMow.transform.localScale;
				otherMow.SendMessage ("setClipRadius", 0.7f);
				GUIManager.powerUpDisplay("RED SHRINKING!!!");
				break;
			case 3:
				manager.SendMessage ("modBlueGrass", "kill");
				GUIManager.powerUpDisplay("BOMBING BLUE GRASS!!!");
				break;
			case 4:
				manager.SendMessage ("modRedGrass", "grow");
				GUIManager.powerUpDisplay("GROWING RED GRASS!!!");
				break;
			}
		}
	}
		...
		foreach(KeyValuePair<Vector3,GameObject> grassKeyValuePair in bgg){
			if(grassKeyValuePair.Value!=null){	//if grass exists
				if (Vector3.Distance(grassKeyValuePair.Key,currentPosition)<clipRadius){ //detect grass
					...
					GameManager.blueScore++; //increment score
					//GUIManager.SetBlueScore(GameManager.blueScore); //update score
	...
	void checkRedGrass(){
		...
		foreach(KeyValuePair<Vector3,GameObject> grassKeyValuePair in rgg){
			if(grassKeyValuePair.Value!=null){	//if grass exists
				if (Vector3.Distance(grassKeyValuePair.Key,currentPosition)<clipRadius){ //detect grass
					GUIManager.mowingOpp(Color.blue);
